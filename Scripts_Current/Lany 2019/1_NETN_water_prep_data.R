...
write.csv(waterDat, file = "~/Google Drive File Stream/My Drive/NETN Water R coding project/NETN_Water_data/NETN_Water_Data.csv", row.names=F)

#select only 'stream', 'epilimnion', or NA Sample Depths.
waterDat2 <- waterDat %>%
	filter(SampleDepth == 'stream' | SampleDepth == 'epilimnion' | is.na(SampleDepth))

waterDat2$Visit.Start.Date <- format(waterDat2$Visit.Start.Date,"%m/%d/%y")
	
#replace "." with " " or "/" in column names to match NPStoret:
names(waterDat2) <- c('NPSTORET Org ID/Code', 'StationID', 'Visit Start Date', 'Depth to Activity', 'Depth.Units','Local Characteristic Name', 'Result Value/Text', 'Lower Quantification Limit', 'Upper Quantification Limit')

#write out data as 'Water Data.csv':
write.csv(waterDat2, file = "~/Google Drive File Stream/My Drive/NETN Water R coding project/NETN_Water_data/Water Data.csv", row.names=F)
...
MD <- MD %>%
	...
	CharacteristicName == "DOsat.pct" ~ "Dissolved Oxygen",
